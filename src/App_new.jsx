...
export default function App() {
  ...
  const loadEntries = async (userId) => {
    try {
      const data = await dbService.getEntries(userId);
      setEntries(data);
    } catch (error) {
  ...
  const handleSocialLogin = async (provider) => {
    try {
      ...
      // OAuth는 리다이렉트되므로 여기서는 추가 처리 불필요
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-2xl font-anton">Loading...</div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
        ...
        {showModal && (
          <AuthModal
            ...
            onEmailSubmit={showModal === 'login' ? handleLogin : handleSignUp}
...
// ============= AUTH MODAL =============
function AuthModal({ type, onClose, onEmailSubmit, onSocialLogin }) {
      ...
      <div className="bg-white border-2 border-black p-8 md:p-12 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-4 bg-white text-gray-500 font-anton">or</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full py-3 px-4 border-2 border-gray-300 hover:bg-gray-50 transition-colors flex items-center justify-center gap-3 text-lg"
          >
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-black text-white hover:bg-gray-800 transition-colors flex items-center justify-center gap-3 text-lg"
          >
...
// 나머지 컴포넌트들은 이전과 동일...
// (YearScreen, MonthScreen, DayScreen, DetailScreen)
